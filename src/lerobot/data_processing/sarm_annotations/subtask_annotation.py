...
def visualize_annotations(
    ...
    for i, ep_idx in enumerate(episodes, 1):
        ...
        try:
            video_path, video_start, video_end, should_cleanup = prepare_episode_video(
                dataset, ep_idx, video_key
            )
        except (FileNotFoundError, ValueError) as e:
            print(f"Could not load episode media for {ep_idx}: {e}")
            continue
        ...
        if video_end is None:
            ...
            video_end = (to_idx - from_idx) / float(dataset.fps)
        ...
        try:
            ...
            if should_cleanup and video_path.exists():
                video_path.unlink()
...
def process_single_episode(
    ...
    try:
        video_path, start, end, should_cleanup = prepare_episode_video(dataset, ep_idx, video_key)
        try:
...
def _normalize_image_frame(frame) -> np.ndarray:
    """Convert a dataset frame into an RGB uint8 numpy array (H, W, C)."""
...
def create_episode_video_from_images(dataset: LeRobotDataset, ep_idx: int, image_key: str) -> Path:
    """Create a temporary MP4 from image-backed episode frames."""
    from_idx = int(dataset.meta.episodes["dataset_from_index"][ep_idx])
    to_idx = int(dataset.meta.episodes["dataset_to_index"][ep_idx])
    if to_idx <= from_idx:
        raise ValueError(f"Episode {ep_idx} has invalid frame range: {from_idx} -> {to_idx}")

    hf_dataset = dataset.hf_dataset.with_format(None)
    episode_dataset = hf_dataset.select_columns([image_key]).select(range(from_idx, to_idx))
    frames = [_normalize_image_frame(item[image_key]) for item in episode_dataset]
    if not frames:
        raise ValueError(f"Episode {ep_idx} contains no frames for {image_key}")

    with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as tmp_file:
        tmp_path = Path(tmp_file.name)

    write_video(tmp_path, frames, dataset.fps)
    return tmp_path


def prepare_episode_video(
    dataset: LeRobotDataset, ep_idx: int, camera_key: str
) -> tuple[Path, float, float | None, bool]:
    """Resolve an episode into a video file for annotation.

    Returns:
        (video_path, start_timestamp, end_timestamp, should_cleanup)
    """
    if camera_key in dataset.meta.video_keys:
        video_path = dataset.root / dataset.meta.get_video_file_path(ep_idx, camera_key)
        if not video_path.exists():
            raise FileNotFoundError(f"Video not found: {video_path}")

        start = float(dataset.meta.episodes[f"videos/{camera_key}/from_timestamp"][ep_idx])
        end = float(dataset.meta.episodes[f"videos/{camera_key}/to_timestamp"][ep_idx])
        return video_path, start, end, False

    if camera_key in dataset.meta.image_keys:
        temp_video_path = create_episode_video_from_images(dataset, ep_idx, camera_key)
        return temp_video_path, 0.0, None, True

    raise ValueError(
        f"Camera key '{camera_key}' is neither video-backed nor image-backed. "
        f"Available camera keys: {dataset.meta.camera_keys}"
    )
...
def worker_process_episodes(
    ...
    for ep_idx in episode_indices:
        _, sparse_ann, err = process_single_episode(
            ep_idx, dataset, video_key, dataset.fps, sparse_annotator
        )
        if sparse_ann:
        ...
        if dense_annotator:
            _, dense_ann, _ = process_single_episode(
                ep_idx, dataset, video_key, dataset.fps, dense_annotator
            )
            if dense_ann:
...
def main():
    ...
    video_key = (
        args.video_key if args.video_key in (dataset.meta.camera_keys or []) else dataset.meta.camera_keys[0]
    )
    storage_type = "video" if video_key in dataset.meta.video_keys else "image"
    print(f"Using camera: {video_key} ({storage_type}-backed), FPS: {fps}")
    ...
    if args.device == "auto":
        if args.num_workers != 1:
            print("Auto device mode enabled: forcing single worker to avoid multi-process OOM.")
        args.num_workers = 1
    ...
    if need_vlm:
        if args.num_workers > 1 and not auto_sparse:
            ...
            for i, ep_idx in enumerate(episode_indices):
                ...
                if sparse_annotator:
                    _, sparse_ann, err = process_single_episode(
                        ep_idx, dataset, video_key, fps, sparse_annotator
                    )
                    if sparse_ann:
                        sparse_annotations[ep_idx] = sparse_ann
                        save_annotations_to_dataset(dataset.root, sparse_annotations, fps, prefix="sparse")
                ...
                if dense_annotator:
                    _, dense_ann, err = process_single_episode(
                        ep_idx, dataset, video_key, fps, dense_annotator
                    )
                    if dense_ann:
                        dense_annotations[ep_idx] = dense_ann
                        save_annotations_to_dataset(dataset.root, dense_annotations, fps, prefix="dense")
