...
def _clip_model_features_to_tensor(features: torch.Tensor | Any) -> torch.Tensor:
    """Normalize CLIP ``get_*_features`` output to a tensor.

    Older ``transformers`` returned a plain tensor; recent versions return
    ...
    if pooler is not None:
        return pooler
    raise TypeError(f"Unexpected CLIP features type: {type(features)}")
...
class SARMEncodingProcessorStep(ProcessorStep):
    ...
    def _encode_images_batch(self, images: np.ndarray) -> torch.Tensor:
        ...
        for i in range(0, num_frames, self.config.clip_batch_size):
            ...
            embeddings = _clip_model_features_to_tensor(self.clip_model.get_image_features(**inputs)).detach().cpu()
    ...
    def _encode_text_clip(self, text: str, batch_size: int) -> torch.Tensor:
        ...
        text_embedding = _clip_model_features_to_tensor(self.clip_model.get_text_features(**inputs)).detach().cpu()
