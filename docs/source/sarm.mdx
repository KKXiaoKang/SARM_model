...
### ARM Upgrade: Tri-state Advantage + AW-BC

This implementation also supports an ARM-style extension:

1. **Tri-state advantage labels** per frame:
   - `0`: Regressive
   - `1`: Stagnant
   - `2`: Progressive
2. **AW-BC weighting** (optional): applies a length-adaptive gain so effective deltas become:

```
ΔG_t = (P_{t+H} - P_t) * (L_seq / L_ref)
```

Where `L_seq` is episode length and `L_ref` is dataset-level reference length.
...
- Adds per-frame advantage deltas and tri-state labels
...
| `--advantage-horizon`  | Frames ahead for advantage delta/label computation             | `1`        |
| `--advantage-positive-threshold` | Delta threshold for progressive label                  | `0.01`     |
| `--advantage-negative-threshold` | Delta threshold for regressive label                   | `-0.01`    |
...
| `episode_length`  | Episode length (for AW-BC length-adaptive gain) |
| `progress_sparse` | Sparse head progress value [0, 1]              |
| `progress_dense`  | Dense head progress value [0, 1] (if computed) |
| `advantage_delta_sparse` | Sparse progress delta using horizon `H`   |
| `advantage_label_sparse` | Sparse tri-state label (0/1/2)            |
| `advantage_delta_dense`  | Dense progress delta (if dense computed)   |
| `advantage_label_dense`  | Dense tri-state label (if dense computed)  |
...
lerobot-train \
  ...
  --rabc_kappa=0.01 \
  --rabc_weighting_mode=rabc \
  --output_dir=outputs/train/policy_rabc \
  --batch_size=32 \
  --steps=40000
```

Example with **AW-BC** (length-adaptive gain):
...
lerobot-train \
  ...
  --use_rabc=true \
  --rabc_head_mode=sparse \
  --rabc_weighting_mode=awbc \
  --rabc_length_adaptive_gain=true \
  --rabc_kappa=0.01 \
  --output_dir=outputs/train/policy_awbc \
  --batch_size=32 \
  --steps=40000
```
...
| `--rabc_weighting_mode` | Weighting backend: `rabc` or `awbc`                       | `rabc`                             |
| `--rabc_length_adaptive_gain` | Enable length-adaptive gain for AW-BC               | `false`                            |
| `--rabc_awbc_ref_length` | Optional fixed `L_ref` (otherwise auto-estimated)        | `null`                             |
...
| `rabc_mean_gain`   | task-dependent| Very high means long episodes dominate |
...
### ARM + AW-BC Practical Notes

1. Use `--policy.reward_mode=arm` when training the reward model (Step 3) to enable tri-state ARM supervision.
2. Keep `rabc_weighting_mode=rabc` as a baseline, then enable `awbc` to test robustness on variable-length DAgger data.
3. If `rabc_mean_gain` is unstable, set `--rabc_awbc_ref_length` explicitly and tune gain clipping.
